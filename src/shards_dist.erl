...
%% @hidden
do_join(Tab) ->
  pg:join(Tab, shards_lib:get_pid(Tab)).

-spec leave(Tab :: atom(), Nodes :: [node()]) -> CurrentNodes :: [node()].
leave(Tab, Nodes) ->
  global:trans({?MODULE, Tab}, fun() ->
    Members = [{node(Pid), Pid} || Pid <- pg:get_members(Tab)],

    ok =
      lists:foreach(fun(Node) ->
        case lists:keyfind(Node, 1, Members) of
          {Node, Pid} -> pg:leave(Tab, Pid);
          _           -> noop
        end
      end, Nodes),
...
get_nodes(Tab) ->
  lists:usort([node(Pid) || Pid <- pg:get_members(Tab)]).
...
rename(Tab, Name, State) ->
  ...
  Nodes = get_nodes(Tab),
  %ok = pg2:delete(Tab),
  [pg:leave(Tab, P) || P <- pg:get_members(Tab)],
  %ok = pg2:create(Name),
